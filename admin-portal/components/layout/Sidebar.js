...
import { AlertTriangle, BarChart2, ClipboardCheck, FileText, LayoutDashboard, LogOut, Settings, Shield, Target, Users } from "lucide-react";
import { useAuth } from "../../context/AuthContext";

const isHrefActive = (href, path) => path === href || path?.startsWith(`${href}/`);

export default function Sidebar() {
  const { user, logout } = useAuth();
  const pathname = usePathname();
  const managerLinks = [["/dashboard", "Dashboard", LayoutDashboard], ["/team", "My Team", Users], ["/checkins", "Check-ins", ClipboardCheck], ["/shared-goals", "Shared Goals", Target], ["/reports", "Reports", FileText]];
  const adminLinks = [["/admin/dashboard", "Dashboard", LayoutDashboard], ["/admin/users", "User Management", Users], ["/admin/cycles", "Cycles", Settings], ["/admin/audit-logs", "Audit Logs", Shield], ["/reports", "Reports", FileText], ["/admin/analytics", "Analytics", BarChart2], ["/admin/escalations", "Escalations", AlertTriangle]];
    ...
    <aside className="fixed left-0 top-0 z-40 flex h-screen w-[220px] flex-col border-r border-white/5 bg-[#0d0d14]">
      <div className="border-b border-white/5 px-5 pb-5 pt-6">
        <div className="flex items-center gap-2.5">
          ...
          <span className="text-[15px] font-semibold tracking-tight text-white">AtomQuest</span>
        </div>
        <p className="ml-9 mt-1.5 text-[10px] uppercase tracking-wider text-slate-500">Admin Portal</p>
      </div>
            ...
            return (
              <Link
                ...
                className={`group relative z-10 flex h-10 items-center gap-3 rounded-lg px-3 text-[13.5px] font-medium transition-colors duration-200 ${
                  active ? "text-indigo-400" : "text-slate-400 hover:bg-white/5 hover:text-slate-200"
                }`}
              >
                <Icon size={15} className={`transition-colors duration-200 ${label === "Escalations" && !active ? "text-amber-500" : active ? "text-indigo-400" : "text-slate-500 group-hover:text-slate-300"}`} />
                {label}
              </Link>
            );
          })}
      ...
      <div className="border-t border-white/5 px-3 pb-4 pt-3">
        <div className="group flex cursor-pointer items-center gap-2.5 rounded-lg px-2 py-2 transition-colors hover:bg-white/5">
          ...
          <button onClick={logout} className="flex-shrink-0 text-slate-600 transition-colors group-hover:text-slate-400" aria-label="Logout">
            <LogOut size={13} />
          </button>
        </div>
      </div>
