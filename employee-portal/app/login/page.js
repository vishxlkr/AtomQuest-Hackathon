...
export default function LoginPage() {
   ...
   const handleMicrosoftLogin = async () => {
      ...
      clearToken();
      try {
         const res = await api.get("/auth/azure/login?portal=employee&prompt=login");
         ...
         // Use window.open to prevent the dev server from interfering
         window.location.href = authUrl;
      } catch (err) {
         setIsMicrosoftLoading(false);
         toast.error(
            err.response?.data?.error?.message ||
               "SSO available in production environment",
         );
      }
   };
      ...
      <main className="flex min-h-screen items-center justify-center bg-[#0a0a0f] px-5 py-10 text-slate-100">
         <section className="w-full max-w-md">
            <div className="mb-6 flex items-center justify-center gap-3">
               <div className="grid h-11 w-11 place-items-center rounded-lg bg-indigo-600 text-white shadow-lg shadow-indigo-200">
                  <Target className="h-6 w-6" />
               </div>
               <div>
                  <p className="text-[13px] text-slate-500">Employee Portal</p>
               </div>
            </div>
            ...
            <div className="rounded-2xl border border-white/[0.08] bg-[#16161f] p-6 shadow-2xl shadow-black/50 sm:p-8">
               ...
               <div className="relative my-6">
                  ...
                  <div className="relative flex justify-center text-xs">
                     <span className="bg-[#16161f] px-3 font-medium uppercase text-slate-600">
                        or
                     </span>
                  </div>
               </div>
