...
import { LayoutDashboard, ListChecks, Bell, LogOut, Target } from "lucide-react";
  ...
  return (
    <aside className="fixed left-0 top-0 z-40 flex h-screen w-[220px] flex-col border-r border-white/5 bg-[#0d0d14]">
      <div className="border-b border-white/5 px-5 pb-5 pt-6">
        ...
        <p className="ml-9 mt-1.5 text-[10px] uppercase tracking-wider text-slate-500">Employee Portal</p>
      </div>
        ...
        <div className="relative space-y-1">
          ...
          {links.map(([href, label, Icon]) => {
            ...
            return (
              ...
              >
                <Icon size={15} className={`transition-colors duration-200 ${active ? "text-indigo-400" : "text-slate-500 group-hover:text-slate-300"}`} />
                {label}
                {label === "Notifications" && unreadCount > 0 ? <span className="ml-auto min-w-[18px] rounded-full bg-indigo-600 px-1.5 py-0.5 text-center text-[10px] font-semibold text-white">{unreadCount}</span> : null}
              </Link>
            );
          })}
        </div>
